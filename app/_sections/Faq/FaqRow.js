  ...
  return (
    <div className="flex gap-4">
      <div className="flex flex-col gap-2">
        <h3 className="text-neutral-900 font-medium text-lg">{question}</h3>
        {isOpen && <p className="text-neutral-600 text-base">{answer}</p>}
      </div>
      <span className="w-6 h-6 shrink-0 ml-auto cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
        {isOpen ? (
